...
class PrinterTextParserColumn(textParserLine: PrinterTextParserLine, textColumn: String) {
    ...
    init {
        ...
        if (!isImgOrBarcodeLine) {
            ...
            line
                .setNbrCharForgetted(nbrCharForgetted).nbrCharColumnExceeded = nbrCharColumnExceeded
        }
    }
    ...
    private fun appendImage(textAlign: String, imageAttributes: Hashtable<String, String>, hexString: String) {
        val printer = this.line.textParser.printer
        val originalBytes = Base64.decode(hexString, Base64.DEFAULT)
        var bitmap = BitmapFactory.decodeByteArray(originalBytes,0,originalBytes.size)
        val originalHeight = bitmap.height
        val originalWidth = bitmap.width
        val size: Int

        if (imageAttributes.containsKey(PrinterTextParser.ATTR_IMAGE_SIZE)) {
            val imageAttribute = imageAttributes[PrinterTextParser.ATTR_IMAGE_SIZE]
            if (imageAttribute != null) {
                size = try {
                    imageAttribute.toInt()
                ...
                bitmap = Bitmap.createScaledBitmap(
                    bitmap,
                    size,
                    (originalHeight.toDouble().roundToInt() * size / (originalWidth)),
                    true
                )
            }
        }
        this.appendElement(PrinterTextParserImg(
            this,
            textAlign,
    ...
    private fun appendBarcode(
        ...
        this.appendElement(PrinterTextParserBarcode(this, textAlign, barcodeAttributes, code))
    }
    ...
    private fun appendQRCode(
        ...
        this.appendElement(PrinterTextParserQRCode(this, textAlign, qrCodeAttributes, data))
    }
    ...
    private fun appendElement(element: IPrinterTextParserElement) {
        val elementsTmp = arrayOfNulls<IPrinterTextParserElement>(elements.size + 1)
